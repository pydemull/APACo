...
#===============================================================================
#' Perform TOSTs for several equivalence bounds
#'
#' @param data A dataframe.
#' @param var A character value to indicate the name of the variable to be plotted.
#' @param group A character value to indcate the name of the variable relating to the tested conditions.
#' @param ref A character value to indicate for which condition the reference mean shoud be computed.
#'
#' @return A table of results.
#' @export
#'

do_t_tost <- function(data, var, group, ref) {
  (purrr::map(c(1, 5, 10, 15, 20), function(perc) {
    # Get the levels of the group variable
    level1 <- levels((data |> dplyr::pull(group)))[[1]]
    level2 <- levels((data |> dplyr::pull(group)))[[2]]

    ## Get the mean of the reference condition
    mean_ref <-
      mean(data |> dplyr::filter(.data[[group]] == ref) |>  dplyr::pull(var))

    # Get the equivalence bound
    equiv_bound <- mean_ref * perc / 100

    # Get the results of the TOST
    res_tost <-
      TOSTER::t_TOST(
        ...
        } := forcats::fct_relevel(.data[[group]], level2, level1)),
        hypothesis = "EQU",
        paired = TRUE,
        var.equal = FALSE,
        eqb = equiv_bound,
    ...
    tab <-
      data.frame(
        lower_ci = res_tost$effsize$lower.ci[1],
        estimate = res_tost$effsize$estimate[1],
        upper_ci = res_tost$effsize$upper.ci[1],
        low_eq = res_tost$eqb$low_eq[1],
        high_eq = res_tost$eqb$high_eq[1],
        decision_tost = res_tost$decision$TOST[1],
        decision_ttest = res_tost$decision$ttest[1]
      ) |>
      dplyr::mutate(perc = perc)

  })) |>
    dplyr::bind_rows() |>
    dplyr::mutate(equiv_zone = paste0("±", perc, "%"))
