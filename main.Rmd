...
# Change in IPAQ-SF MET-min/week between 6 and 12 months
## Descriptive statistics
```{r IPAQ_desc_stats}
...
# Change in the motivational profile for physical activity
...
**Comment**: We removed AMOTIVATION variable that caused an error due to an IQR of 0.
...
## Visualize the final clusters from K-Medoid approach
```{r get_final_clusters_emaps}
targets::tar_read(final_clusters_emaps_viz)
```

## Visualize the descriptive statistics of the EMAPS scores related to the motivational profiles
```{r viz_clusters, fig.width=15, fig.cap = "AU = Autonomous; IR = Introjected regulation."}
...
## Compare the motivational profiles
...
## Characterize the change in the motivational profile
### Visualize the change in the motivational profile
...
# Predict 6WT distance trajectory using latent class mixed modelling
## Build the models
...
## Visualize the fixed effetcs of the model
```{r plot_preds_6MWT}
...
# Predict IPAQ-SF MET-min/week trajectory using latent class mixed modelling
## Build the models
...
**Comment**: The 4-class and the 5-class did not converge. The 2-class model seemed to be a reasonable choice.
