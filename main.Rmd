...
## Choose the best mixed model with a random intercept for further anaysis
### Build the models
```{r load_simple_mixed_models_6MWT}
# Load the model summarises
tar_load(model_6MWT_1a)
tar_load(model_6MWT_1b)
tar_load(model_6MWT_1c)
tar_load(model_6MWT_1d)
```

#### Model 1a: MONTH + random intercept
```{r}
summary(model_6MWT_1a)
```

#### Model 1b: model 1a + MONTH * 6MWT_M0 + MONTH * MET_MIN_WK_M0 + MONTH * MOTIVATION_CLUSTER_M0
```{r}
summary(model_6MWT_1b)
```

#### Model 1c: MONTH * 6MWT_M0 + MONTH * MET_MIN_WK_M0 + MONTH * MOTIVATION_CLUSTER_M0 + sex + age
```{r}
summary(model_6MWT_1c)
```

#### Model 1d: model 1c + meteo_defavorable (unfavourable weather) + manque_temps (lack of time)
```{r}
summary(model_6MWT_1d)
```

### Compare the models
```{r compare_simple_mixed_models_6MWT}
tar_read(compa_simple_mixed_models_6MWT)
```

Comment: The model 1b, based on its AIC and BIC, is retained for further analysis.

## Compare the random-intercept and the random-slope mixed models
### Model 1e: model 1b + random slope for MONTH
```{r random_slope_mixed_model_6MWT}
# Load the model
tar_load(model_6MWT_1e)

# Model summary
summary(model_6MWT_1e)
```

Comment: No p value can be provided when adding a random slope term to the initial model (the model did not converge). The initial model 1b with only a random intercept is kept for further analysis

## Choose the best latent class mixed model for further anaysis
### Build the models
```{r load_latent_class_models_6MWT}
tar_load(model_6MWT_2a)
tar_load(model_6MWT_2b)
tar_load(model_6MWT_2c)
tar_load(model_6MWT_2d)
```

#### Model 2a: model 1b + 2 classes
```{r}
summary(model_6MWT_2a)
```

####  Model 2b: model 1b + 3 classes
```{r}
summary(model_6MWT_2b)
```
####  Model 2c: model 1b + 4 classes
```{r}
summary(model_6MWT_2c)
```
####  Model 2d: model 1b + 5 classes
```{r}
summary(model_6MWT_2d)
```
...
##### Model 2a
```{r}
lcmm::postprob(model_6MWT_2a)
```
##### Model 2b
```{r}
lcmm::postprob(model_6MWT_2b)
```

##### Model 2c
```{r}
lcmm::postprob(model_6MWT_2c)
```
##### Model 2d
```{r}
lcmm::postprob(model_6MWT_2d)
```

Comment: Based on the metrics shown above, the model with 5 classes seems the most appropriate model.

### Assess the chosen model
```{r plot_model_6MWT_2d_resids}
plot(model_6MWT_2d)
```
...
## Choose the best mixed model with a random intercept for further anaysis
### Build the models
```{r load_simple_mixed_models_IPAQ}
# Load the model summarises
tar_load(model_IPAQ_1a)
tar_load(model_IPAQ_1b)
tar_load(model_IPAQ_1c)
tar_load(model_IPAQ_1d)
```

#### Model 1a: MONTH + random intercept
```{r}
summary(model_IPAQ_1a)
```

#### Model 1b: model 1a + MONTH * 6MWT_M0 + MONTH * MET_MIN_WK_M0 + MONTH * MOTIVATION_CLUSTER_M0
```{r}
summary(model_IPAQ_1b)
```

#### Model 1c: MONTH * 6MWT_M0 + MONTH * MET_MIN_WK_M0 + MONTH * MOTIVATION_CLUSTER_M0 + sex + age
```{r}
summary(model_IPAQ_1c)
```

#### Model 1d: model 1c + meteo_defavorable (unfavourable weather) + manque_temps (lack of time)
```{r}
summary(model_IPAQ_1d)
```

### Compare the models
```{r compare_simple_mixed_models_IPAQ}
tar_read(compa_simple_mixed_models_IPAQ)
```
...
## Compare the random-intercept and the random-slope mixed models
### Model 1e: model 1b + random slope for MONTH
```{r random_slope_mixed_model_IPAQ}
# Load the model
tar_load(model_IPAQ_1e)

# Model summary
summary(model_IPAQ_1e)

# Compare models 1b and 1e
tar_read(compa_rand_mixed_models_IPAQ)
```

Comment: The addition of random slope for MONTH does not add a lot of value. The simplest model is then chosen: model 1b.


## Choose the best latent class mixed model for further anaysis
### Build the models
```{r load_latent_class_models_IPAQ}
tar_load(model_IPAQ_2a)
tar_load(model_IPAQ_2b)
tar_load(model_IPAQ_2c)
tar_load(model_IPAQ_2d)
```

#### Model 2a: model 1b + 2 classes
```{r}
summary(model_IPAQ_2a)
```

####  Model 2b: model 1b + 3 classes
```{r}
summary(model_IPAQ_2b)
```
####  Model 2c: model 1b + 4 classes
```{r}
summary(model_IPAQ_2c)
```
####  Model 2d: model 1b + 5 classes
```{r}
summary(model_IPAQ_2d)
```
...
##### Model 2a
```{r}
lcmm::postprob(model_IPAQ_2a)
```
##### Model 2b
```{r}
lcmm::postprob(model_IPAQ_2b)
```

##### Model 2c
```{r}
lcmm::postprob(model_IPAQ_2c)
```
##### Model 2d
```{r}
lcmm::postprob(model_IPAQ_2d)
```

Comment: Only the model with 2 classes could provide p values (the other models did not converge). The model 2a is thus chosen.

### Assess the chosen model
```{r plot_model_IPAQ_2a_resids}
plot(model_6MWT_2a)
```
