...
* Step 6: Ensure you have an internet connexion and build the required Docker image (the place where will be created the files embedding the results) by running the following command line (do not forget the point at the end of the line):
```{r, eval=FALSE}
...
* Step 7: Create a Docker container to move the files embedding the results from the Docker image to your machine, and more precisely to the folder named `shared_folder` that is inside the unzipped folder named `APACo-pipeline`. To do this, use the following command line:
```{r, eval=FALSE}
...
In the example above (adapted for a Windows user),`C:/Users/USER_NAME/Downloads/APACo-pipeline/shared_folder`) is the absolute path to the folder named `shared_folder` at the root of the unzipped folder named `APACo-pipeline`. As previoulsy described, you should replace `C:/Users/USER_NAME/Downloads/` by the path that fits your needs.This is the only part of the code you should modify.


Step 8: Finally, go to the `shared_folder` folder at the root of the initially unzipped folder (i.e., inside the `APACo-pipeline` folder). The results embedded in .tiff figures and .html files should be there. You can now close the Command Prompt (or Terminal) app.
